...
current_shell_name="$(basename "/"$(ps -o cmd -f -p $(cat /proc/$(echo $$)/stat | cut -d \  -f 4) | tail -1 | sed 's/ .*$//'))"

if [ "$current_shell_name" = "yakuake" ] || [ "$current_shell_name" = "edex-ui" ]; then
  unimatrix -s 95 -w
fi
...
alias edex="/home/quincy/Downloads/eDEX-UI-Linux-x86_64.AppImage"

alias update="sudo apt update && sudo apt upgrade && sudo apt-get update && sudo apt-get upgrade"

export PATH=$PATH:/home/quincy/.spicetify
